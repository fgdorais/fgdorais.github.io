...
^
$$R^\times :\equiv\\{x:R \mid \operatorname{\mathsf{unit}}(x)\\} :\equiv\sum_{x:R} \operatorname{\mathsf{unit}}(x).$$
^
...
^
$$R^\times = \\{(x,y):R \times R \mid x \cdot y =_R \mathsf{I}\\} = \sum_{x,y:R} (x \cdot y =_R \mathsf{I}).$$
^
